...
@Component({
  selector: 'app-maptalks',
  templateUrl: './maptalks.component.html',
  styleUrls: ['./maptalks.component.css']
})
export class MaptalksComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    new E3Layer('e3', this.getECOption()).addTo(map);
  }
  ...
  getECOption() {
    ...
    [['北京', BJData], ['上海', SHData], ['广州', GZData]].forEach(
      function (item, i) {
        series.push(trailLine(item, i), airPlan(item, i), station(item, i));
      });
